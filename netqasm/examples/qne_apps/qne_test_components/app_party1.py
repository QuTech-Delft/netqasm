...
def main(app_config=None):
    ...
    with party1:
        app_logger.log(
            "Starting with local gate operations: init, X, H, Z, RotZ(pi/4), RotX(pi/2)"
        )
        q0 = Qubit(party1)
        party1.flush()
        ...
        party1.flush()
        app_logger.log(
            "Following gates: init qubit 1, CNOT(0, 1), CNOT(1, 0), CPHASE(0, 1), CPHASE(1, 0)"
        )
        ...
        party1.flush()
        app_logger.log(
            "Following gates: measure qubit 2, measure qubit 0, measure qubit 1"
        )
        ...
        socket.send(
            "This is a simple message. After this follows a message with header and payload."
        )
        msg = StructuredMessage(
            header="This is a header", payload="This is the payload"
        )
        socket.send_structured(msg)
        app_logger.log(
            "These classical messages and app logs should not remove the qubit state visualizations"
        )
        ...
        party1.flush()
        app_logger.log(
            "Following operation: CNOT(0, 1). After that, there should be a 4-qubit group across two nodes"
        )
        epr1.cnot(epr2)

    table_2_columns = []
    for i in range(10):
        table_2_columns.append([i * 1, i * 2])

    table_6_columns = []
    for i in range(10):
        table_6_columns.append(
            [
                f"some long text {i * 1000}",
                i,
                i + 1,
                i + 2,
                i + 3,
                f"even looooonger text {i * 11111111}",
            ]
        )
